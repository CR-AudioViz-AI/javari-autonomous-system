...
export function BrandedHeader({ appName, appLogo, quickLinks = [] }: BrandedHeaderProps) {
  ...
  const checkAuthStatus = async () => {
    try {
      ...
      // session.data IS the User object directly (not session.data.user)
      if (session.success && session.data) {
        ...
        if (creditsResult.success) {
          ...
          // API returns 'tier', map to plan
          const tier = creditsResult.data?.tier || 'free';
          setPlan(tier as 'free' | 'pro' | 'business');
        }
      }
    } catch (error) {

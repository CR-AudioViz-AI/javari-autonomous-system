 ...
 * @param source - Source of the learning content (e.g., 'devdocs', 'fcc', 'mdn')
 * @param contentType - Type of content (e.g., 'documentation', 'tutorial', 'news')
 * @param content - JSON string of content data
 * @param priority - Priority level (higher = processed first, default 5)
 */
...
/**
 * Upsert a knowledge entry to the javari_knowledge_base table
...
export async function upsertKnowledge(entry: KnowledgeEntry): Promise<void> {
  try {
    ...
    if (error) {
      console.error('[upsertKnowledge] Failed to upsert knowledge:', error.message);
    }
  } catch (err) {
